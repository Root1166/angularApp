$(function() {
    ...
    $(function() {
        ...
        $("ul#sidebarnav a").click(function () {
            $("ul#sidebarnav a").removeClass('active');
            $(this).addClass('active');
        });
        $('.date-picker').bootstrapMaterialDatePicker({ format: 'DD-MM-YYYY', time: false });
    }), jQuery(document).on("click", ".mega-dropdown", function(e) {
        e.stopPropagation();
    });
    var set = function() {
        (window.innerWidth > 0 ? window.innerWidth : this.screen.width) < 1170 ? ($("body").addClass("mini-sidebar"), 
            $(".navbar-brand span").hide(), $(".navbar-brand b img").css('width','50px').css('margin-left','0px'), $(".sidebartoggler i").addClass("ti-menu")) : ($("body").removeClass("mini-sidebar"), 
                $(".navbar-brand span").show(), $(".navbar-brand b img").css('width','100px').css('margin-left','20px'));
    ...
    $(window).ready(set), $(window).on("resize", set), $(".sidebartoggler").on("click", function() {
        $("body").hasClass("mini-sidebar") ? ($("body").trigger("resize"), $("body").removeClass("mini-sidebar"), 
            $(".navbar-brand span").show(), $(".navbar-brand b img").css('width', '100px').css('margin-left', '20px')) : ($("body").trigger("resize"), $("body").addClass("mini-sidebar"), 
                $(".navbar-brand span").hide(), $(".navbar-brand b img").css('width', '50px').css('margin-left', '0px'));
