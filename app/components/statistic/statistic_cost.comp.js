angular.module('statistic-cost.comp', ['common.srv']).
    component('statisticCost', {
        templateUrl: '/app/components/statistic/statistic_cost.tpl.html',
        controller: ['$routeParams', '$scope', '$rootScope', '$location', 'commonService','baocaoApi','userApi',
            function ($routeParams, $scope, $rootScope, $location, commonService,baocaoApi,userApi) {
                ...
                this.onLoad = function() {
                    baocaoApi.reportCost(self.filter).then(function(response) {
                        console.log(response);
                        var response = response.data
                        if (response.result) {
                            self.reportCostModel = response.data;
                        } else {
