angular.module('statistic-progress.comp', ['common.srv']).
    component('statisticProgress', {
        templateUrl: '/app/components/statistic/statistic_progress.tpl.html',
        controller: ['$routeParams', '$scope', '$rootScope', '$location', 'commonService', 'thanhphoApi', 'quanhuyenApi', 'baocaoApi',
            function($routeParams, $scope, $rootScope, $location, commonService, thanhphoApi, quanhuyenApi, baocaoApi) {
                ...
                this.onLoad = function() {
                    baocaoApi.reportProgress(self.filter).then(function(response){
                        if (response.data) {
                            self.reportProgressModels = response.data.data;
                        } else {
