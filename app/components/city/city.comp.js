...
angular.module('city.comp', ['city.api', 'common.srv']).
    component('city', {
        templateUrl: '/app/components/city/city.tpl.html',
        controller: ['$routeParams', '$scope', '$rootScope', '$location', 'cityApi', 'commonService',
            function ($routeParams, $scope, $rootScope, $location, cityApi, commonService) {
                var self = this;
                self.cityModels = [];
                self.currentPage = 1;
                self.pageSize = '50';
                ...
                this.onRemoves = function (city) {
                    ...
                    commonService.confirmSweet('Chắc chắn xóa?', 'Bạn sẽ không thể khôi phục bản ghi sau khi xóa!', function (isConfirm) {
                        if (isConfirm) {
                            ...
                            //remove
                            cityApi.delete(selectedCitys).then(function (response) {
                ...
                this.onRemove = function (city) {
                    ...
                    commonService.confirmSweet('Chắc chắn xóa?', 'Bạn sẽ không thể khôi phục bản ghi sau khi xóa!', function (isConfirm) {
                        if (isConfirm) {
                            //remove
                            cityApi.delete([city.Id]).then(function (response) {
                                var response = response.data;
                                if (response.result) {
                                    self.filter.Paging.CurrentPage = 1;
                                    self.filter.Paging.PageSize = self.pageSize;
